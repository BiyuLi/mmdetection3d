...
class NuScenesMonoDatasetHPP(CocoDataset):
    ...
    CLASSES = ('car', 'truck', 'trailer', 'bus', 'construction_vehicle',
               ...
               'barrier','vehicle_head', 'vehicle_rear')
    VEHICLE_CAT= ('car', 'truck', 'trailer', 'bus', 'construction_vehicle', 'bicycle', 'motorcycle')
    PERSON_CAT=('pedestrian')
    RIDER_ATTR=('cycle.with_rider') # rider没有单独检测，通过交通工具的attribute_name来区分，包含with_rider则是汽车人
    REAR_CAT=('vehicle_head', 'vehicle_rear')
    DefaultAttribute = {
        'car': 'vehicle.parked',
    ...
    def pre_pipeline(self, results):
        ...
        results['img_fields'] = []
        results['vehicle_fields'] = []
        results['person_fields'] = []
        results['rider_fields'] = []
        results['rear_fields'] = []
        results['ignore_fields'] = []
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                gt_bboxes_ignore.append(bbox)
                continue
            bbox_cam3d = np.array(ann['bbox_cam3d']).reshape(1, -1).squeeze()
            center2d = ann['center2d'][:2]
            if ann['category_name'] in self.VEHICLE_CAT \
                and ann['attribute_name'] not in self.RIDER_ATTR:
                vehicle_bboxes_gt.append(bbox)
                vehicle_bboxes3d_gt.append(bbox_cam3d)
                vehicle_centers2d_gt.append(center2d)

            if ann['category_name'] in self.PERSON_CAT:
                person_bboxes_gt.append(bbox)
                person_bboxes3d_gt.append(bbox_cam3d)
                person_centers2d_gt.append(center2d)

            if ann['attribute_name'] in self.RIDER_ATTR:
                rider_bboxes_gt.append(bbox)
                rider_bboxes3d_gt.append(bbox_cam3d)
                rider_centers2d_gt.append(center2d)

            if ann['category_name'] in self.REAR_CAT:
                rear_bboxes_gt.append(bbox)
    
        # convert gts to np.array
        # 如果当前图片没有该种类的gt，用np.zeros()占位

        # vehicle branch
        # 2d
        if vehicle_bboxes_gt:
            vehicle_bboxes_gt = np.array(vehicle_bboxes_gt, dtype=np.float32)
        else:
            vehicle_bboxes_gt = np.zeros((0, 4), dtype=np.float32)
        # 3d
        if vehicle_bboxes3d_gt:
            vehicle_bboxes3d_gt = np.array(vehicle_bboxes3d_gt, dtype=np.float32)
            vehicle_centers2d_gt = np.array(vehicle_centers2d_gt, dtype=np.float32)
        else:
            vehicle_bboxes3d_gt = np.zeros((0, self.bbox_code_size),
                                       dtype=np.float32)
            vehicle_centers2d_gt = np.zeros((0, 2), dtype=np.float32)

        # person branch
        # 2d
        if person_bboxes_gt:
            person_bboxes_gt = np.array(person_bboxes_gt, dtype=np.float32)
        else:
            person_bboxes_gt = np.zeros((0, 4), dtype=np.float32)
        # 3d
        if person_bboxes3d_gt:
            person_bboxes3d_gt = np.array(person_bboxes3d_gt, dtype=np.float32)
            person_centers2d_gt = np.array(person_centers2d_gt, dtype=np.float32)
        else:
            person_bboxes3d_gt = np.zeros((0, self.bbox_code_size),
                                       dtype=np.float32)
            person_centers2d_gt = np.zeros((0, 2), dtype=np.float32)

        # rider branch
        # 2d
        if rider_bboxes_gt:
            rider_bboxes_gt = np.array(rider_bboxes_gt, dtype=np.float32)
        else:
            rider_bboxes_gt = np.zeros((0, 4), dtype=np.float32)
        # 3d
        if rider_bboxes3d_gt:
            rider_bboxes3d_gt = np.array(rider_bboxes3d_gt, dtype=np.float32)
            rider_centers2d_gt = np.array(rider_centers2d_gt, dtype=np.float32)
        else:
            rider_bboxes3d_gt = np.zeros((0, self.bbox_code_size),
                                       dtype=np.float32)
            rider_centers2d_gt = np.zeros((0, 2), dtype=np.float32)

        # rear branch
        if rear_bboxes_gt:
            rear_bboxes_gt = np.array(rear_bboxes_gt, dtype=np.float32)
        else:
            rear_bboxes_gt = np.zeros((0, 4), dtype=np.float32)
        ...
        vehicle_gt = [vehicle_bboxes_gt,vehicle_bboxes3d_gt,vehicle_centers2d_gt]
        person_gt = [person_bboxes_gt, person_bboxes3d_gt, person_centers2d_gt]
        rider_gt = [rider_bboxes_gt, rider_bboxes3d_gt, rider_centers2d_gt]
        rear_gt = [rear_bboxes_gt]
    ...
    def get_attr_name(self, attr_idx, label_name):
        ...
        if label_name == 'car' or label_name == 'bus' \
            ...
            if AttrMapping_rev2[attr_idx] == 'vehicle.moving' or \
                ...
                return NuScenesMonoDatasetHPP.DefaultAttribute[label_name]
